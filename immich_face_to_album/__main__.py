...
def _get_assets_for_person_single_type(
    ...
    while True:
        ...
        if verbose:
            click.echo(
                ...
                f"(page {page}, created_after={created_after}, asset_type={asset_type})"
            )
...
def get_assets_for_person(
    ...
    types = list(asset_type) if asset_type else [None]

    merged = {}
    for t in types:
        for item in _get_assets_for_person_single_type(
            server_url,
            key,
            person_ids,
    ...
    raw = json.dumps(
        {
            ...
            "asset_type": sorted(asset_type) if asset_type else [],
        },
        sort_keys=True,
    )
...
@click.option(
    ...
    multiple=True,
    help="Only include assets of this type. Can be used multiple times (default: all types).",
)
@click.option(
    "--remove-non-matching",
    is_flag=True,
