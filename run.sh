...
run_job() {
  ...
  local -a type_args=()
  local asset_type_image asset_type_video asset_type_audio asset_type_other
  asset_type_image="$(echo "${job}" | jq -r '.asset_type_image')"
  asset_type_video="$(echo "${job}" | jq -r '.asset_type_video')"
  asset_type_audio="$(echo "${job}" | jq -r '.asset_type_audio')"
  asset_type_other="$(echo "${job}" | jq -r '.asset_type_other')"
  [ "${asset_type_image}" = "true" ] && type_args+=(--asset-type IMAGE)
  [ "${asset_type_video}" = "true" ] && type_args+=(--asset-type VIDEO)
  [ "${asset_type_audio}" = "true" ] && type_args+=(--asset-type AUDIO)
  [ "${asset_type_other}" = "true" ] && type_args+=(--asset-type OTHER)
