## v1.0.17
- Merged with [v1.0.17](https://github.com/romainrbr/immich-face-to-album/releases/tag/1.0.17) of the original app.
- Added `asset_type` filter: each job can now optionally restrict sync to a specific media type (`IMAGE`, `VIDEO`, `AUDIO`, `OTHER`). If all four are off, all types are synced
- `asset_type` is included in the config hash, so changing it automatically triggers a full re-scan for that job.
- `asset_type` is passed to all `get_assets_for_person` calls, including skip-face lookups.
